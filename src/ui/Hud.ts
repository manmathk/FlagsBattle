...
const EVENT_LABELS: Record<ChaosEventKind, string> = { vortex: '🌀 Vortex', wind: '💨 Wind', speedBurst: '⚡ Speed burst', chaosSpin: '🌪️ Chaos spin' };
const flagBackground = (code: string, size: number): string => {
  ...
  return [`background-image:url(${import.meta.env.BASE_URL}atlas/flags.png)`, `background-size:${atlas.size.w * k}px ${atlas.size.h * k}px`, `background-position:-${frame.x * k}px -${frame.y * k}px`].join(';');
};
...
export class Hud {
  ...
  countryName(flagCode: string): string { return nameOf(flagCode); }

  applyTheme(theme: Theme): void {
    const root = document.documentElement.style;
    root.setProperty('--bg-outer', toCss(theme.bg[0])); root.setProperty('--bg-inner', toCss(theme.bg[1])); root.setProperty('--accent', toCss(theme.accent)); root.setProperty('--glow', toCss(theme.glow));
    if (theme.light) {
      root.setProperty('--text', '#0f172a'); root.setProperty('--muted', '#64748b'); root.setProperty('--panel', 'rgb(255 255 255 / 84%)'); root.setProperty('--border', 'rgb(15 23 42 / 12%)'); root.setProperty('--control-bg', 'rgb(15 23 42 / 6%)'); root.setProperty('--control-hover', 'rgb(15 23 42 / 11%)'); root.setProperty('--option-bg', '#ffffff');
    } else {
      root.setProperty('--text', '#f8fafc'); root.setProperty('--muted', '#94a3b8'); root.setProperty('--panel', 'rgb(8 8 16 / 62%)'); root.setProperty('--border', 'rgb(255 255 255 / 10%)'); root.setProperty('--control-bg', 'rgb(255 255 255 / 7%)'); root.setProperty('--control-hover', 'rgb(255 255 255 / 14%)'); root.setProperty('--option-bg', '#0b0b12');
    }
  }
  ...
  setTopFive(standings: readonly Standing[], roundsPlayed: number): void {
    ...
    if (standings.length === 0) {
      const empty = document.createElement('li'); empty.className = 'strip-empty'; empty.textContent = 'Waiting for the first champion…'; this.topList.replaceChildren(empty); return;
    }
    this.topList.replaceChildren(...standings.slice(0, 5).map((standing, index) => {
      const item = document.createElement('li'); item.className = `winner-item winner-rank-${index + 1}`; item.title = `${index + 1}. ${nameOf(standing.flagCode)} — ${standing.wins} ${standing.wins === 1 ? 'win' : 'wins'}`;
      const rank = document.createElement('span'); rank.className = 'winner-rank'; rank.textContent = `${index + 1}`;
      const chip = document.createElement('span'); chip.className = 'winner-flag'; chip.setAttribute('style', flagBackground(standing.flagCode, 30));
      const info = document.createElement('span'); info.className = 'winner-info';
      const name = document.createElement('span'); name.className = 'winner-name'; name.textContent = nameOf(standing.flagCode);
      const wins = document.createElement('span'); wins.className = 'winner-wins'; wins.textContent = `${standing.wins} ${standing.wins === 1 ? 'win' : 'wins'}`;
      info.append(name, wins); item.append(rank, chip, info); return item;
    }));
  }
  setSeries(standings: readonly Standing[], target: number): void {
    if (standings.length === 0) { const empty = document.createElement('li'); empty.className = 'strip-empty'; empty.textContent = `first to ${target}`; this.seriesList.replaceChildren(empty); return; }
  ...
  showWinner(flagCode: string, kicker: string, isChampion: boolean, holdMs: number): void {
    ...
    this.banner.className = isChampion ? 'banner champion' : 'banner'; this.banner.replaceChildren(kickerEl, flagEl, nameEl); this.banner.hidden = false;
    window.clearTimeout(this.bannerTimer); this.bannerTimer = window.setTimeout(() => { this.banner.hidden = true; }, holdMs);
  }
  hideBanner(): void { window.clearTimeout(this.bannerTimer); this.banner.hidden = true; }
}
